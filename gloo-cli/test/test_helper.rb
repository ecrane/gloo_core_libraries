...
# be gem installed instead of the code in this working tree. It would
# also re-register these same objects a second time, which trips a
# pre-existing bug in Gloo::Core::Dictionary#add_object/#add_verb
# (Dictionary#@engine is never assigned, so its duplicate-registration
# error path always raises NoMethodError - normally masked by
# LibManager's own broad rescue).
#
require 'gloo-cli'
